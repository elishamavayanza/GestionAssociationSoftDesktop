...
public class DonPanel extends JPanel implements Refreshable {
    ...
    private JTextArea descriptionField; // Champ supplémentaire pour la description
    private Contribution contributionEnEdition;

    public DonPanel(Long membreId, ContributionManager contributionManager) {
    ...
    private void initComponents() {
        ...
        // Titre
        JLabel titleLabel = new JLabel("Dons");
        ...
        descriptionLabel.setFont(Fonts.labelFont());
        formPanel.add(descriptionLabel, gbc);

        gbc.gridy = 2;
        descriptionField = new JTextArea(3, 40);
        descriptionField.setFont(Fonts.textFieldFont());
        ...
        historyTableModel = new DefaultTableModel(
                new Object[]{"Date", "Montant", "Description", "Actions"}, 0) {
            @Override
    ...
    private void customizeTableAppearance() {
        ...
        for (int i = 0; i < historyTable.getColumnCount(); i++) {
            if (i != 2) { // La colonne description n'est pas centrée
        ...
        historyTable.getColumnModel().getColumn(2).setPreferredWidth(300); // Description
    ...
    private void submitContribution(ActionEvent e) {
        try {
            ...
            if (contributionEnEdition != null) {
                ...
                contributionEnEdition.setDescription(description);
    ...
    private class TableActionCellEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        public TableActionCellEditor() {
            ...
            editButton.addActionListener(e -> {
                if (currentRow >= 0 && currentRow < historyTableModel.getRowCount()) {
        ...
        public Component getTableCellEditorComponent(JTable table, Object value,
            ...
            // Vérification robuste des limites
            if (row < 0 || row >= table.getModel().getRowCount()) {
                System.err.println("Tentative d'édition d'une ligne invalide: " + row);
                return new JPanel();
            }
        ...
        public void fireEditingStopped() {
            // Vérifier que currentRow est toujours valide
            if (currentRow >= 0 && currentRow < historyTableModel.getRowCount()) {
    ...
    private void editContribution(int row) {
        try {
            ...
            if (row < 0 || row >= historyTableModel.getRowCount()) {
                return;
            }
            // Récupérer la contribution correspondante à la ligne
            List<Contribution> contributions = contributionManager.findByMembreAndType(
                    membreId, TypeContribution.DON);
            ...
            if (row >= 0 && row < contributions.size()) {
                ...
                descriptionField.setText(contributionEnEdition.getDescription());
    ...
    private void deleteContribution(int row) {
        try {
            // Vérification plus robuste des limites
            if (row < 0 || row >= historyTableModel.getRowCount()) {
                System.err.println("Tentative de suppression d'une ligne invalide: " + row);
                return;
            }

            // Arrêter l'édition en cours avant toute suppression
            if (historyTable.isEditing()) {
                TableCellEditor editor = historyTable.getCellEditor();
                if (editor != null) {
            ...
            Object dateObj = historyTableModel.getValueAt(row, 0);
            Object amountObj = historyTableModel.getValueAt(row, 1);

            if (dateObj == null || amountObj == null) {
                System.err.println("Données manquantes pour la suppression");
                return;
            }
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                boolean success = contributionManager.supprimerContribution(
                        membreId, amount, date, TypeContribution.DON.name());

                if (success) {
                    // Vérifier à nouveau les limites avant suppression
                    if (row >= 0 && row < historyTableModel.getRowCount()) {
                        historyTableModel.removeRow(row);
                    }

                    // Réinitialiser si c'était la contribution en édition
                    if (contributionEnEdition != null &&
